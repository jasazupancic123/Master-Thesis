...
export function formatDate(date: Dayjs | Date, options = { withYear: true }): string {
  return dayjs(date).format(`DD. MM.${options.withYear ? ' YYYY' : ''}`);
}

export function formatTime(date: Dayjs | Date): string {
...
export function getWeekDays(day = dayjs()): Day[] {
  // start from monday
