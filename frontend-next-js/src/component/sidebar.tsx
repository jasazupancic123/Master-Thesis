...
export default function Sidebar({title}: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      {title && <Box display='flex' justifyContent='center' bgcolor='#000' pb={3}>
        <Box bgcolor='#303E4A' borderRadius='0 0 50px 50px'>
          <Typography variant='h5' component='h1' py={1} px={8}>{title}</Typography>
        </Box>
      </Box>}

      <Box
        sx={{position: 'fixed', top: 0, left: 0, height: '64px', width: '64px'}}
        onClick={toggle(true)}
      >
        <Box
          sx={{
