...
import { ExerciseAttributeRef, ExerciseRef } from '../../common/type/firebase-firestore.type';
...
export class ExerciseAttributeValueService {
   ...
   */
  async findAllAsObject(ref: Required<ExerciseRef>): Promise<Record<string, any>> {
    const attributeValues = await this.exerciseAttributeValueRepository.getDocs(ref);
    await Promise.all(
      attributeValues.map(async attributeValue => {
        attributeValue.attribute = await this.exerciseAttributeRepository.getDoc(attributeValue.attributeId);
      }),
    );
   ...
   */
  async createMany(ref: Required<ExerciseRef>, input: Record<string, any>): Promise<void> {
    ...
    const attributeValues: ExerciseAttributeValue[] = await Promise.all(
      Object.entries(input).map(async ([field, value]) => {
        ...
        if (!this.commonService.object.isValidValue(attribute, value))
          throw new BadRequestException(`Invalid attribute value ${JSON.stringify({ [field]: value })}, possible values: ${JSON.stringify(attribute.values)}`);
    ...
    for (const attributeValue of attributeValues) {
      const attributeValueRef = { ...attributeValue, uid: ref.uid };
      const document = this.exerciseAttributeValueRepository.doc(attributeValueRef);
      batch.set(document, attributeValue);
    }
  ...
  async updateMany(ref: Required<ExerciseAttributeRef>): Promise<void> {
    // find attribute
    const attribute = await this.exerciseAttributeRepository.getDoc(ref.attributeId);
    if (!attribute)
      throw new BadRequestException('Attribute not found');
    ...
    const attributeValues = (
      await this.exerciseRepository
        .collectionGroup('attributes')
        .where(FieldPath.documentId(), '==', ref.attributeId)
        .get()
    ).docs.map(doc => {
