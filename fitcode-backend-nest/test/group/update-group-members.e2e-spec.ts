...
describe('Update Group (e2e)', () => {
  ...
  async function addAthleteReq(groupId: string, token: string, userId: string) {
    return await testApp.http.patch(
      `/institution/${institutionId}/group/${groupId}/athlete`,
      token,
      { userId },
    );
  }
  ...
  async function addTrainerReq(groupId: string, token: string, userId: string) {
    return await testApp.http.patch(
      `/institution/${institutionId}/group/${groupId}/trainer`,
      token,
      { userId },
    );
  }
  ...
  describe('Athlete Member Updates', () => {
    ...
    it('should fail if member is not part of the institution', async () => {
      ...
      expect(response.body.message).toBe(
        'Member is not part of the institution',
      );
  ...
  describe('Trainer Member Updates', () => {
    it('should successfully add a trainer to the group', async () => {
      const newTrainer = await testApp.auth.createTrainer();
      await db.institutions.members.addMember(
      ...
      const group = await db.groups.findById({ institutionId, groupId });
      expect(group.trainerIds).toContain(newTrainer.uid);
      expect(group.trainerIds).toHaveLength(2);

      // manually remove the trainer from the group
      await testApp.auth.deleteUsers([newTrainer.uid]);
      await db.groups.removeTrainer({ institutionId, groupId }, newTrainer.uid);
    });

    it('should successfully remove a trainer from the group', async () => {
      ...
      const group = await db.groups.findById({ institutionId, groupId });
      expect(group.trainerIds).not.toContain(global.trainer.uid);
      expect(group.trainerIds).toHaveLength(0);

      // manually add the trainer back to the group
      await db.groups.addTrainer(
