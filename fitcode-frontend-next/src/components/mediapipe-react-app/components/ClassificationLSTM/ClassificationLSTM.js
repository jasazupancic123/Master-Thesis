...
const ClassificationLSTM = ({ poseResults, videoRef, canvasRef }) => {
  ...
  const updateLandmarkBuffer = useCallback(
    (newLandmarks) => {
  ...
  useEffect(() => {
    ...
    console.log(
      'Processing poseResults.poseLandmarks:',
      poseResults.poseLandmarks
    ); // Debugging statement

    const landmarks = ESSENTIAL_LANDMARKS.map(
      (index) => poseResults.poseLandmarks[index] || { x: 0, y: 0, z: 0 }
    );
    ...
    if (normalizedLandmarks) {
      ...
      if (landmarkBuffer.length === bufferSize) {
        ...
        const inputTensor = tf.tensor3d(inputBuffer, [
          1,
          bufferSize,
          ESSENTIAL_LANDMARKS.length * 3,
        ]);
