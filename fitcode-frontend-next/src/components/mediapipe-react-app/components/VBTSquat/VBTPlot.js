...
const variance = (records, m) =>
  mean(records.map((record) => (record - m) ** 2));
const covariance = (records1, mean1, records2, mean2) =>
  mean(records1.map((_, i) => (records1[i] - mean1) * (records2[i] - mean2)));
...
export const VBTPlot = ({
  upTimesHistory,
  downTimesHistory,
  isometricTimesHistory,
}) => {
  ...
  useEffect(() => {
    ...
    const { slope, intercept } = coEfficient(
      xValues.map((_, index) => index + 1),
      upTimesHistory
    );
    setFatigueIndex(slope);
    const regressionLineY = xValues.map(
