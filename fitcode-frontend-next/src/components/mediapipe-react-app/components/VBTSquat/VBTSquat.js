...
import { calculate3DDistance, calculate2DDistance, calculateAngles, calculateStandardDeviation, calculateStandardDeviation1D } from '../../utils/Calculations';
...
const VBTSquat = ({ poseResults, videoRef, canvasRef }) => {
    ...
    const [phaseDurations, setPhaseDurations] = useState({ eccentric: 0, isometric: 0, concentric: 0 });
    ...
    const updateHipAnkleDistances = (newDistance) => {
        hipAnkleDistances.current = [...hipAnkleDistances.current.slice(-BUFFER_SIZE + 1), newDistance];
    ...
    const handleSquatCompletion = useCallback(() => {
        if (!squatStartTimestampRef.current || !squatLowestTimestampRef.current || !squatEndTimestampRef.current) {
            console.error("Error: One or more squat timestamps are undefined.");
            return;
        }

        const totalTime = squatEndTimestampRef.current - squatStartTimestampRef.current;
        const eccentricTime = squatLowestTimestampRef.current - squatStartTimestampRef.current;
        const concentricTime = squatEndTimestampRef.current - squatLowestTimestampRef.current;
        const totalIsometricTime = isometricTotalTimeRef.current;

        if (totalTime <= 300 || eccentricTime <= 0 || concentricTime <= 0) {
            console.error("Error: Squat times are not valid. Total time or phase times are not within valid ranges.");
            return;
        }
        ...
        const adjustedEccentricTime = eccentricTime - (totalIsometricTime * eccentricProportion);
        const adjustedConcentricTime = concentricTime - (totalIsometricTime * concentricProportion);
    ...
    const updateStandingReferences = (isCurrentlyStill, leftHipAnkleDistance, rightHipAnkleDistance, results) => {
        if (isCurrentlyStill) {
            const standingDistance = Math.max(leftHipAnkleDistance, rightHipAnkleDistance);
    ...
    useEffect(() => {
        ...
        const updateSquatStatus = (isCurrentlyStill, results, currentTime) => {
            const { newLeftKneeAngle, newRightKneeAngle, newLeftHipAngle, newRightHipAngle } = calculateAngles(results);
            ...
            const currentShortestDistance = Math.min(leftHipAnkleDistance, rightHipAnkleDistance);
            updateHipAnkleDistances(currentShortestDistance);

            if (isCurrentlyInSquat && !prevIsInSquatRef.current && isReadyRef.current) {
                // START
            ...
            } else if (!isCurrentlyInSquat && prevIsInSquatRef.current && isReadyRef.current) {
                // END
            ...
            if (isCurrentlyInSquat && isReadyRef.current) {
                if (currentShortestDistance < shortestDistanceRef.current) {
                    ...
                    const stdDev = calculateStandardDeviation1D(hipAnkleDistances.current);
                    if (stdDev < ISOMETRIC_MOVEMENT_THRESHOLD) {
                        if (squatPhase !== 'isometry') {
                            ...
                            isometricTotalTimeRef.current += currentTime - isometricStartTimestampRef.current;
                            isometricStartTimestampRef.current = currentTime;
                        }
                    } else {
                        if (squatPhase === 'isometry') {
                ...
                updateStandingReferences(isCurrentlyStill, leftHipAnkleDistance, rightHipAnkleDistance, results);
        ...
        if (poseResults.poseLandmarks) {
            const allLandmarksVisible = ESSENTIAL_SQUAT_LANDMARKS.every(index =>
    ...
    }, [poseResults, feedbackGenerator, handleSquatCompletion, isLowestPointDetected, isInSquat, squatPhase]);

    return (
        <div>
            <div className="vbt-live-data-container">
                Squat Counter: {squatCounter}<br />
