...
const Classification = ({ poseResults, videoRef, canvasRef }) => {
  ...
  const updatePredictionBuffer = useCallback(
    (newPrediction) => {
      setPredictionBuffer((prevBuffer) => {
        const buffer =
          prevBuffer.length >= bufferSize ? prevBuffer.slice(1) : prevBuffer;
  ...
  useEffect(() => {
    ...
    const visibleCount = ESSENTIAL_SQUAT_LANDMARKS.reduce(
      (count, index) =>
        poseResults.poseLandmarks[index] &&
        poseResults.poseLandmarks[index].visibility > 0.3
          ? count + 1
          : count,
      0
    );
    ...
    if (visibleCount >= minVisibleCount) {
      ...
      const prediction = model.predict(
        inputTensor.reshape([1, normalizedLandmarks.length * 4])
      );
