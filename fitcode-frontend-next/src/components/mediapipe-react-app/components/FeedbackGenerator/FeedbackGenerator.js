import { calculate3DDistance, pointToPlaneDistance, calculatePlaneNormal, calculateVectorAngle, calculateShinAngle, calculateThighAngle } from "../../utils/Calculations";
...
export class FeedbackGenerator {
    ...
    analyzePose(poseCoordinates, isCurrentlyInSquat) {
        ...
        if (!isCurrentlyInSquat) {
            ...
            // }
            // check shins
            if (verticalAngles.leftShin > Thresholds.TRUNK_VERTICAL_THRESH || verticalAngles.leftShin > Thresholds.TRUNK_VERTICAL_THRESH) {
    ...
    checkKneeAlignment(poseCoordinates) {
        ...
        const leftNormal = calculatePlaneNormal(leftHipToAnkleVector, leftAnkleToeVector);
        const rightNormal = calculatePlaneNormal(rightHipToAnkleVector, rightAnkleToeVector);

        // Multiply the left distance by -1 to make negative distance indicate valgus for both legs
        const leftKneeDistance = -1 * pointToPlaneDistance(leftKnee, leftAnkle, leftNormal);
        const rightKneeDistance = pointToPlaneDistance(rightKnee, rightAnkle, rightNormal);
