...
export class SquatMetrics {
  ...
  getSquatType() {
    ...
    if (this.squatDepth <= 45) return depthFeedback.replace('%s', 'full squat');
      ...
      return depthFeedback.replace('%s', 'half squat');
    else return depthFeedback.replace('%s', 'quarter squat');
  }

  getMovementRhythm() {
    const rhythmFeedback = 'The rhythm of your movement was %s.';
    if (this.ascentTime < 600)
      return rhythmFeedback.replace('%s', 'quite reactive');
    else if (this.ascentTime <= 700)
      return rhythmFeedback.replace('%s', 'dynamic');
    else if (this.ascentTime <= 1000)
      return rhythmFeedback.replace('%s', 'controlled');
  ...
  getDescentPhase() {
    ...
    return this.descentTime < 500
      ...
      : descentFeedback.replace(
          '%s',
          'not significantly loading eccentric contraction regime'
        );
  }
  ...
  getKneeAlignment() {
    ...
    return this.kneeAlignmentDepthPercentage > 150
      ...
      : alignmentFeedback.replace(
          '%s',
          'quite narrow, potentially not hitting optimal depth'
        );
  }

  getLimbLoading() {
    const loadingFeedback =
      'In regards to symmetrical limb loading, this was a %s.';
    if (this.com >= -0.034 && this.com <= 0.034)
      return loadingFeedback.replace('%s', 'well-aligned squat');
    else if (this.com >= 0.035 && this.com <= 0.045)
      return loadingFeedback.replace('%s', 'slightly left leg dominant squat');
    else if (this.com <= -0.035 && this.com >= -0.045)
      return loadingFeedback.replace('%s', 'slightly right leg dominant squat');
    else if (this.com > 0.046)
      return loadingFeedback.replace('%s', 'heavily left leg dominant squat');
    else
      return loadingFeedback.replace('%s', 'heavily right leg dominant squat');
  }
