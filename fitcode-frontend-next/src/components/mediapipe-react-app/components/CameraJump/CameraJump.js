...
const CameraJump = ({ poseResults, videoRef, canvasRef }) => {
  ...
  const detectTPose = useCallback(() => {
    ...
    const leftShoulder =
      poseResults.poseLandmarks[POSE_LANDMARKS.LEFT_SHOULDER];
    const rightShoulder =
      poseResults.poseLandmarks[POSE_LANDMARKS.RIGHT_SHOULDER];
  ...
  const checkAnklesLiftOff = useCallback(() => {
    if (
      leftAnklePositions.current.length < 2 ||
      rightAnklePositions.current.length < 2
    ) {
    ...
    const leftAnkleDiff =
      leftAnklePositions.current[leftAnklePositions.current.length - 1].y -
      leftAnklePositions.current[leftAnklePositions.current.length - 2].y;
    const rightAnkleDiff =
      rightAnklePositions.current[rightAnklePositions.current.length - 1].y -
      rightAnklePositions.current[rightAnklePositions.current.length - 2].y;

    // Check if both ankles have a decrease in y value of 0.01 or more
    return (
      leftAnkleDiff <= -ANKLE_Y_THRESH && rightAnkleDiff <= -ANKLE_Y_THRESH
    );
  }, []);
  ...
  useEffect(() => {
    ...
    if (poseResults.poseLandmarks) {
      const allLandmarksVisible = ESSENTIAL_JUMP_LANDMARKS.every(
        (index) =>
      ...
      if (allLandmarksVisible) {
        ...
        const isCurrentlyStill =
          stdDevLeft < MOVEMENT_THRESHOLD && stdDevRight < MOVEMENT_THRESHOLD;
        ...
        // update hip y value
        const leftHipPosition =
          poseResults.poseLandmarks[POSE_LANDMARKS.LEFT_HIP];
        const rightHipPosition =
          poseResults.poseLandmarks[POSE_LANDMARKS.RIGHT_HIP];
        const currentHipY = calculateHipMidpointY(
          leftHipPosition,
          rightHipPosition,
        ...
        if (!jumpActive) {
          ...
          if (isCurrentlyStill) {
            ...
            if (!isCurrentlyInSquat && currentHipY < referenceHipY) {
              ...
              const scalingFactor = calculateCmPerPixel(
                userHeightRef.current.value,
          ...
          if (jumpPhase === JUMP_PHASE.LANDING) {
            ...
            if (isCurrentlyStill && !isCurrentlyInSquat) {
              ...
              const newScalingFactor = calculateCmPerPixel(
                userHeightRef.current.value,
                poseResults.poseLandmarks,
                videoRef.current.videoHeight
              );
              ...
              const cmYDiff =
                jumpHeightPx * avgScalingFactor -
                adjustmentConstantRef.current.value;
    ...
    <div>
      ...
      <div className="user-height-input-container">
        ...
        <input
          ...
          min="0"
        />
        <label htmlFor="adjustmentConstant" style={{ marginLeft: '10px' }}>
          Adjustment (cm):{' '}
        </label>
        <input
          type="number"
