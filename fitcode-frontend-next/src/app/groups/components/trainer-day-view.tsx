...
export default function TrainerDayView(props: GroupPageProps) {
  ...
  async function addSuperset(
    ...
    try {
      const superset = await TrainingController.addSuperset(
        token,
        trainingId,
        componentId,
        input
            ...
            ? {
                ...
                components: training.components?.map((component) =>
                  component.componentId === componentId
                    ? {
                        ...component,
                        supersets: [...(component.supersets || []), superset],
                      }
                    : component
                ),
              }
  ...
  async function updateExercise(
    ...
    try {
      const exercise = await TrainingController.updateExercise(
        token,
        trainingId,
        componentId,
        supersetId,
  ...
  useEffect(() => {
    async function fetchTrainings() {
      ...
      try {
        const trainings = await TrainingController.findTrainingsByIds(
          token,
          trainingIds
        );
        setTrainings(
              ...
              {training?.components?.map((component, i) => {
                return (
                  <Fragment key={i}>
                    {/* Exercise list */}
                    {exercises.show &&
                      exercises.componentId === component.componentId &&
                      exercises.supersetId && (
                        <Stack>
                          <Stack
                            direction="row"
                            spacing={1}
                    ...
                    >
                      ...
                      >
                        ...
                        <IconButton
                          size="small"
                          onClick={() =>
                            deleteComponent(training.id, component.componentId)
                          }
                          sx={{ mr: 1 }}
                        >
                          <DeleteIcon />
                      ...
                      <Box bgcolor="background.paper" p={2}>
                        ...
                        <Grid container spacing={2} wrap="wrap">
                          {component?.supersets
                            ...
                            ?.map((superset, i) => {
                              return (
                                <Grid xs={6} key={superset.id} spacing={3}>
                                  <BorderColor
                                  ...
                                  <Box>
                                    {superset.exercises.map((exercise, k) => (
                                      <Box
                                        key={`${superset.id}-${exercise.exerciseId}-${k}`}
                                        position="relative"
                                      >
                                        <Box
                                          position="absolute"
